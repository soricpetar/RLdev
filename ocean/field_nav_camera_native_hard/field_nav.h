...
typedef struct FieldNav {
    ...
    float heading_alignment_scale;
    float goal_outside_fov_penalty;
    float world_size_m;
    float dt;
    ...
    float blocked_corridor_prob;
    float blocked_corridor_wall_prob;
    float blocked_corridor_min_distance_m;
    float blocked_corridor_max_distance_m;
    float blocked_corridor_min_half_width_m;
    float blocked_corridor_max_half_width_m;
...
static inline void fn_add_blocked_corridor(FieldNav* env) {
    if (fn_rand01(env) >= env->blocked_corridor_prob) return;

    float dx = env->goal_x - env->robot_x;
    float dy = env->goal_y - env->robot_y;
    float goal_dist = sqrtf(dx * dx + dy * dy);
    if (goal_dist < 8.0f) return;

    float ux = dx / goal_dist;
    float uy = dy / goal_dist;
    float px = -uy;
    float py = ux;
    float max_block_dist = fminf(env->blocked_corridor_max_distance_m, goal_dist - 3.0f);
    if (max_block_dist < env->blocked_corridor_min_distance_m) return;

    float distance = fn_uniform(env, env->blocked_corridor_min_distance_m, max_block_dist);
    float half_width = fn_uniform(env, env->blocked_corridor_min_half_width_m, env->blocked_corridor_max_half_width_m);
    float lateral_shift = fn_uniform(env, -0.8f, 0.8f);
    float cx = env->robot_x + ux * distance + px * lateral_shift;
    float cy = env->robot_y + uy * distance + py * lateral_shift;

    if (fn_rand01(env) < env->blocked_corridor_wall_prob) {
        fn_add_wall(
            env,
            cx - px * half_width,
            cy - py * half_width,
    ...
    float spacing = fn_uniform(env, 1.05f, 1.35f);
    for (float offset = -half_width; offset <= half_width + 0.001f; offset += spacing) {
        fn_add_circle(
            env,
            FIELD_NAV_TREE,
            cx + px * offset + fn_normal(env, 0.0f, 0.15f),
            cy + py * offset + fn_normal(env, 0.0f, 0.15f),
            0.55f,
            0.0f,
            0.0f
        );
...
static inline void fn_compute_observation(FieldNav* env) {
    ...
    float half_fov = 0.5f * env->front_camera_fov_deg * FIELD_NAV_PI / 180.0f;
    for (int ab = 0; ab < env->polar_angle_bins; ab++) {
        float angle = -FIELD_NAV_PI + ((float)ab + 0.5f) * FIELD_NAV_TWO_PI / (float)env->polar_angle_bins;
        if (fabsf(angle) <= half_fov) continue;
        for (int db = 0; db < env->polar_distance_bins; db++) {
            int cell = ab * env->polar_distance_bins + db;
            obs[cell] = 0.0f;
            obs[map_cells + cell] = 0.0f;
            obs[2 * map_cells + cell] = 0.0f;
            obs[3 * map_cells + cell] = 1.0f;
        }
    }
...
static inline void c_step(FieldNav* env) {
    ...
    int max_action = env->speed_control ? 14 : 4;
    if (action < 0) action = 0;
    if (action > max_action) action = max_action;
    int steering_idx = env->speed_control ? action / 3 : action;
    float steering = steering_values[steering_idx];
    if (env->speed_control) {
        float throttle = throttle_values[action % 3];
        if (throttle > 0.0f) {
    ...
    float goal_heading = atan2f(env->goal_y - env->robot_y, env->goal_x - env->robot_x);
    float heading_error = fn_wrap_pi(goal_heading - env->heading);
    ...
    reward += env->heading_alignment_scale * cosf(heading_error);
    if (env->goal_outside_fov_penalty > 0.0f) {
        float half_fov = 0.5f * env->front_camera_fov_deg * FIELD_NAV_PI / 180.0f;
        if (fabsf(heading_error) > half_fov) {
            float speed_norm = env->speed / fmaxf(1e-6f, env->max_speed_mps);
            reward -= env->goal_outside_fov_penalty * fn_clip(speed_norm, 0.0f, 1.0f);
        }
    }
...
static inline void c_render(FieldNav* env) {
    printf(
        "field_nav_camera step=%d pos=(%.2f,%.2f) heading=%.2f goal_dist=%.2f objects=%d\n",
        env->steps,
        env->robot_x,
        env->robot_y,
        env->heading,
