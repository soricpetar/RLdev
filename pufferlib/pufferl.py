    ...
    def write_live_log(stage, flat_logs=None, checkpoint_path=None):
        ...
        payload = {
            ...
            'config': {
                ...
                'total_agents': args['vec']['total_agents'],
                'horizon': args['train']['horizon'],
                'lr_warmup_steps': args['train'].get('lr_warmup_steps', 0),
                'curriculum_enabled': args.get('env', {}).get('curriculum_enabled', 0),
                'curriculum_warmup_steps': args.get('env', {}).get('curriculum_warmup_steps', 0),
                'experiment_name': args['train'].get('experiment_name', ''),
                'experiment_description': args['train'].get('experiment_description', ''),
                'front_camera_observation': args.get('env', {}).get('front_camera_observation', 0),
                'front_camera_fov_deg': args.get('env', {}).get('front_camera_fov_deg', 0),
            },
            'latest': flat_logs,
